...
枚举值可以除了使用静态值（常数项），还可以使用`计算值`（比如：函数返回值，变化值）。如果存在计算值属性，则该属性必然是在具备`initializer 初始化器`的属性的前面，或者是在最后。换句话说针对不带`initializer`的属性，其可以在第一位，or 数字常量 or 具备`initializer`的属性之后。
...
常量枚举不允许包含计算成员。在编译阶段将会被删除，同时内联进使用的地方。
...
类其实可以看作另一种对象，所以使用`interface`去修饰它。当然，和一般的对象类型不同，需要关键字`implements`来辅助修饰。
...
合并时，如果具备重名属性，则判断类型是否一致，不一致将报错。对于方法的合并，和函数重载一致
